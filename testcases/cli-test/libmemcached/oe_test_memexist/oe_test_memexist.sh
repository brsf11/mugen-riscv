...
# @Desc      :   verify the uasge of memexist command
# ############################################
...
function run_test() {
    ...
    memexist --help | grep "-"
    CHECK_RESULT $?
    memexist --version | grep "memexist"
    CHECK_RESULT $?
    expect <<EOF
...
EOF
    ...
    CHECK_RESULT $?
    memexist --debug --servers=127.0.0.1 user_id user_name | grep -E "Found key user_id|Found key user_name"
    CHECK_RESULT $?
