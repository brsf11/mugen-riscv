...
function run_test() {
    ...
    CHECK_RESULT $?
    test "$(rst2html4 -V | awk '{print$3}')" == "$(rpm -qa python3-docutils | awk -F "-" '{print$3}')"
    CHECK_RESULT $?
    rst2html4 -h | grep 'Usage'
    CHECK_RESULT $?
    rst2html4 --template=template_html.txt testfile.rst test5.html
    CHECK_RESULT $?
    grep '<table class="docinfo"' test5.html
    CHECK_RESULT $? 1
    rst2html4 --initial-header-level=2 testfile.rst test6.html
    CHECK_RESULT $?
    grep '<h1>' test6.html
    CHECK_RESULT $? 1
    rst2html4 --footnote-references=superscript testfile.rst test7.html
    CHECK_RESULT $?
    grep '<sup>1' test7.html
    CHECK_RESULT $?
    rst2html4 --attribution=none testfile.rst test8.html
    CHECK_RESULT $?
    grep '<p class="attribution">Buckaroo Banzai' test8.html
    CHECK_RESULT $?
    rst2html4 --no-compact-lists testfile.rst test9.html
    CHECK_RESULT $?
    grep '<p class="first">' test9.html
    CHECK_RESULT $?
    rst2html4 --table-style=collapse testfile.rst test10.html
    CHECK_RESULT $?
    grep 'collapse' test10.html
    CHECK_RESULT $?
    rst2html4 --no-xml-declaration testfile.rst test11.html
    CHECK_RESULT $?
