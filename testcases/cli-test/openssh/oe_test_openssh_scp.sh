...
function run_test() {
    ...
    expect <<EOF
        spawn scp ${NODE2_USER}@${NODE2_IPV4}:/etc/openEuler-latest /tmp/openEuler-latest
        expect {
            "*yes/no*" {
                send "yes\\r"
            }
...
EOF
    ...
    CHECK_RESULT $?
    expect <<EOF
        spawn scp /etc/openEuler-latest ${NODE2_USER}@${NODE2_IPV4}:/tmp/openEuler-latest
        expect {
            "*yes/no*" {
                send "yes\\r"
            }
