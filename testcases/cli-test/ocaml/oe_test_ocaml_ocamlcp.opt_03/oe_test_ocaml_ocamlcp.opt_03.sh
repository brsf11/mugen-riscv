...
function run_test() {
    ...
    ocamlcp.opt -dllib /usr/lib64/libbfd-2.34.so
    CHECK_RESULT $?
    grep -a "/usr/lib64/libbfd-2.34" a.out
    CHECK_RESULT $?
    ocamlcp.opt -dllpath ../ a.c
    CHECK_RESULT $?
    test -f a.o && rm -rf a.o
    CHECK_RESULT $?
    ocamlcp.opt -dtypes example.ml
    CHECK_RESULT $?
    grep -A 3 "type" example.annot
    CHECK_RESULT $?
    ocamlcp.opt -for-pack P -c example.ml
    CHECK_RESULT $?
