...
function run_test() {
    ...
    ocamlmktop.byte --help | grep "ocamlc"
    CHECK_RESULT $?
    ocamlmktop.byte -a -o a.o a.c
    CHECK_RESULT $?
    grep -a "bytecomp_c_compiler" a.o
    CHECK_RESULT $?
    ocamlmktop.byte -custom -o hello.exe hello.ml hello_stubs.c
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    ocamlmktop.byte -bin-annot example.ml
    CHECK_RESULT $?
    ocamlcmt -info example.cmt | grep "module name" -A 15
    CHECK_RESULT $?
    ocamlmktop.byte -c a.c
    CHECK_RESULT $?
    grep -ai "editor" a.o
    CHECK_RESULT $?
    ocamlmktop.byte -color auto a.c
    CHECK_RESULT $?
    grep -a "rela.eh_frame" a.o
    CHECK_RESULT $?
    ocamlmktop.byte -absname file.ml >result 2>&1
    CHECK_RESULT $? 0 1
    grep "/file.ml" result
    CHECK_RESULT $?
    ocamlmktop.byte -config a.c | grep -E "version|ocamlc" -A 55
    CHECK_RESULT $?
