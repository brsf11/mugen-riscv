...
function run_test() {
    ...
    CHECK_RESULT $?
    ocamlcp -custom -o hello.exe hello.ml hello_stubs.c
    CHECK_RESULT $?
    ./hello.exe | grep "Hello world!"
    CHECK_RESULT $?
    ocamlcp -annot example.ml
    CHECK_RESULT $?
    ./a.out | grep "6"
    CHECK_RESULT $?
    ocamlcp -bin-annot example.ml
    CHECK_RESULT $?
    ...
    grep -ai "editor" a.o
    CHECK_RESULT $?
    ocamlcp -color auto a.c
    CHECK_RESULT $?
    grep -a "rela.eh_frame" a.o
    CHECK_RESULT $?
    ocamlcp -absname hello.ml >result 2>&1
    CHECK_RESULT $? 0 1
    grep "/hello.ml" result
    CHECK_RESULT $?
    ocamlcp -config a.c | grep -E "version|ocamlc" -A 55
    CHECK_RESULT $?
    ocamlcp -annot example.ml
    rm -rf a.out
    ocamlcp -compat-32 example.cmo
    CHECK_RESULT $?
    ./a.out | grep 6
    CHECK_RESULT $?
