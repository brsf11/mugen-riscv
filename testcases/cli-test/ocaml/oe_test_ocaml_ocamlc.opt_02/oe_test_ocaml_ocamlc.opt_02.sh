...
function run_test() {
    ...
    rm -rf a.out
    ocamlc.opt -compat-32 example.cmo
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    ocamlc.opt -dllpath ../ a.c
    CHECK_RESULT $?
    test -f a.o && rm -rf a.o
    CHECK_RESULT $?
    ocamlc.opt -dtypes example.ml
    CHECK_RESULT $?
    grep -A 3 "type" example.annot
    CHECK_RESULT $?
    ocamlc.opt -for-pack P -c example.ml
    CHECK_RESULT $?
