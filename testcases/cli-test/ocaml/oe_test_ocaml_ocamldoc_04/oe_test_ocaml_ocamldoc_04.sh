...
function run_test() {
    ...
    ocamldoc -latex -notoc example.ml
    CHECK_RESULT $?
    grep "tableofcontents" ocamldoc.out
    CHECK_RESULT $? 1
    ocamldoc -latex -notrailer example.ml
    CHECK_RESULT $?
    grep "end{document}" ocamldoc.out
    CHECK_RESULT $? 1
    ocamldoc -latex -sepfiles example.ml
    CHECK_RESULT $?
    grep -Ei "module|example" Example.tex
    CHECK_RESULT $?
    ocamldoc -texi -esc8 example.ml
    CHECK_RESULT $?
    grep "Indices" ocamldoc.texi
    CHECK_RESULT $?
    ocamldoc -texi -info-entry example.ml
    CHECK_RESULT $?
    grep "@direntry"$'\n'"example.ml"$'\n'"@end direntry" ocamldoc.texi
    CHECK_RESULT $?
    ocamldoc -texi -info-section example.ml
    CHECK_RESULT $?
    grep "@dircategory example.ml" ocamldoc.texi
    CHECK_RESULT $?
    ocamldoc -texi -noheader example.ml
    CHECK_RESULT $?
    grep "start of header" ocamldoc.texi
    CHECK_RESULT $? 1
    ocamldoc -texi -noindex example.ml
    CHECK_RESULT $?
    grep "index" ocamldoc.texi
    CHECK_RESULT $? 1
    ocamldoc -texi -notrailer example.ml
    CHECK_RESULT $?
