...
function run_test() {
    ...
    ocamlmklib.opt -v -cclib c example.ml | grep "ocamlc -a" | grep cclib
    CHECK_RESULT $?
    ocamlmklib.opt -v -ccopt c example.ml | grep "ocamlc -a" | grep ccopt
    CHECK_RESULT $?
    ocamlmklib.opt -v -custom example.o | grep "gcc"
    CHECK_RESULT $? 1
    ...
    CHECK_RESULT $?
    ocamlmklib.opt -v -I /tmp example.ml | grep "tmp"
    CHECK_RESULT $?
