...
function run_test() {
    ...
    ocamlmktop -intf-suffix mli lazy.mli
    CHECK_RESULT $?
    grep -ai "lazy" lazy.cmi
    CHECK_RESULT $?
    ocamlmktop -alias-deps example.ml
    CHECK_RESULT $?
    grep -ai "alias" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -keep-locs example.ml
    CHECK_RESULT $?
    grep -ai "locs" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -app-funct example.ml
    CHECK_RESULT $?
    grep -ai "app-funct" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -labels example.ml
    CHECK_RESULT $?
    grep -ai "labels" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -linkall example.ml
    CHECK_RESULT $?
    grep -ai "linkall" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -keep-docs example.ml
    CHECK_RESULT $?
    grep -ai "docs" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -safe-string example.ml
    CHECK_RESULT $?
    grep -ai "safe" a.out && rm -rf a.out
    CHECK_RESULT $?
    ocamlmktop -open Printf example.ml
    CHECK_RESULT $?
