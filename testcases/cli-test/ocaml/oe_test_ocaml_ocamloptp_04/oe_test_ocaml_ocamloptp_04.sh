...
function run_test() {
    ...
    ocamloptp -dtypes example.ml
    CHECK_RESULT $?
    grep -A 3 "type" example.annot
    CHECK_RESULT $?
    ocamloptp -for-pack P -c example.ml
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    cp ../hello_stubs.c ./
    ocamloptp -i hello_stubs.c
    CHECK_RESULT $?
    objdump -x hello_stubs.o | grep "caml_print_hello"
    CHECK_RESULT $?
    ocamloptp -I +/usr/lib64/ocaml hello_stubs.c
    CHECK_RESULT $?
    grep -ai "hello world" hello_stubs.o
    CHECK_RESULT $?
    ocamloptp -v a.c | grep -E "version|Standard library directory"
    CHECK_RESULT $?
    ocaml_version=$(rpm -qa ocaml | awk -F '-' '{print $2}')
    ocamloptp -vnum example.ml | grep $ocaml_version
    CHECK_RESULT $?
    ocamloptp -version example.ml | grep $ocaml_version
    CHECK_RESULT $?
