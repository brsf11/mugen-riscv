...
#@Desc          :   verification ipvsadm‘s DR_wrr model
#####################################
...
function pre_test() {
    ...
    ipvsadm -a -t $VIP:80 -r ${NODE2_IPV4}:80 -g -w 2
    ipvsadm -a -t $VIP:80 -r ${NODE3_IPV4}:80 -g -w 1
    SLEEP_WAIT 10
...
function run_test() {
    ...
    CHECK_RESULT "$(grep -cE "${NODE2_IPV4}" ./result_curl.txt)" 4
    CHECK_RESULT "$(grep -cE "${NODE3_IPV4}" ./result_curl.txt)" 2
