...
#@Desc      	:   verification ipvsadm‘s DEL command
#####################################
...
function run_test() {
    ...
    SLEEP_WAIT 2
    ipvsadm -d -t $VIP:80 -r ${NODE2_IPV4}:80
    CHECK_RESULT "$(ipvsadm -L | grep -cE 'Route')" 1
    CHECK_RESULT "$(ipvsadm -L | grep -cE 'http')" 2
    ipvsadm -D -t $VIP:80
    CHECK_RESULT "$(ipvsadm -L | grep -cE 'http')" 0
