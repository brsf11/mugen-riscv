...
# @Desc      :   Test corosync-qdevice.service restart
# #############################################
...
function pre_test() {
    ...
    P_SSH_CMD --node 2 --cmd "dnf install -y "dnf install -y corosync-qdevice corosync-qnetd"
    ...
    P_SSH_CMD --node 3 --cmd "dnf install -y corosync pacemaker pcs corosync-qdevice;
...
function post_test() {
    ...
    dnf remove -y corosync pacemaker pcs corosync-qdevice;
    mv /etc/hosts_bak /etc/hosts"
    DNF_REMOVE
    P_SSH_CMD --node 2 --cmd "dnf install -y "dnf remove -y corosync-qdevice corosync-qnetd"
    ha_post
