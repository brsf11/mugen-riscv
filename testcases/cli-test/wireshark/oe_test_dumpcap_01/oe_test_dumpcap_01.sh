...
function run_test() {
    ...
    CHECK_RESULT $?
    dumpcap --version | grep "Dumpcap (Wireshark)"
    CHECK_RESULT $?
    dumpcap -D | grep "[0-9]"
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    dumpcap -i $netCard -s 1 -c 20 -w testfile3
    CHECK_RESULT $?
    ...
    CHECK_RESULT $?
    dumpcap -i $netCard -L | grep "Data link types of interface"
    CHECK_RESULT $?
...
EOF
    ...
    CHECK_RESULT $?
    dumpcap -M -D | grep -E "[0-9]|network"
    CHECK_RESULT $?
    dumpcap -M -L | grep -E "Ethernet|DOCSIS"
    CHECK_RESULT $?
    expect <<EOF
        log_file log2
        spawn dumpcap -M -S
