...
function run_test() {
    ...
    expect "sqlite>"
    send "select * from COMPANY WHERE ID = 7;\n"
    expect "sqlite>"
    send "DELETE FROM COMPANY WHERE ID = 7;\n"
    expect "sqlite>"
    send "select * from COMPANY WHERE ID = 7;\n"
    expect "sqlite>"
    send ".quit\n"
    ...
    exit
END
    CHECK_RESULT "$(awk 'NR==1' ../common/output.txt | grep -cE "7")" 1
    CHECK_RESULT "$(wc -l ../common/output.txt | grep -cE "1")" 1
