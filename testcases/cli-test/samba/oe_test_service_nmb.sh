...
function run_test() {
    ...
    CHECK_RESULT $? 0 0 "nmb.service reload failed"
    systemctl status nmb.service | grep "Active: active"
    CHECK_RESULT $? 0 0 "nmb.service reload causes the service status to change"
