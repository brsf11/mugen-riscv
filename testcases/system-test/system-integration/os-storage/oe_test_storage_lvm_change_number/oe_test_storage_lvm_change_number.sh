...
# @Desc      :   Change the number of mirrors for the raid volume
# ############################################
...
"
    ...
    set timeout 30
    log_file testlog
    spawn lvconvert -m2 openeulertest/test /dev/${local_disk3}
    expect \"*\[y/n\]*\" {send \"y\r\"}
    expect eof
"
...
function post_test() {
    ...
    pvremove /dev/${local_disk1} /dev/${local_disk} /dev/${local_disk3}
    rm -rf testlog
