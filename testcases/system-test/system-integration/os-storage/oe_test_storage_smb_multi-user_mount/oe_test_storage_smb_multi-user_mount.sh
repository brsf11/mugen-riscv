...
# @Desc      :   Perform multi-user SMB mount
# #############################################
...
function pre_test() {
    ...
    mkdir -p /home/client
    touch /root/smb.cred
    echo "username=testsamba" >>/root/smb.cred
    echo "password=${NODE1_PASSWORD}" >>/root/smb.cred
    chmod 600 /root/smb.cred
    cp -a /etc/fstab /etc/fstab.bak
    echo "//${NODE2_IPV4}/testsamba /home/client cifs multiuser,sec=ntlmssp,credentials=/root/smb.cred  0 0" >>/etc/fstab
...
function run_test() {
    ...
    expect -c "
        log_file testlog
		spawn cifscreds add -u SMB_user_name ${NODE2_IPV4}
		expect \"*assword*\" 
