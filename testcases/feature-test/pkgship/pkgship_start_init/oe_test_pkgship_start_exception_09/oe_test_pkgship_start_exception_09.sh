...
function run_test() {
    ...
    log_path"
    ACT_SERVICE stop
    CHECK_RESULT $? 0 0 "Check start by systemctl failed when set log_path=''."
    ...
    sed -i '/log_path/d' ${SYS_CONF_PATH}/package.ini
    systemctl start pkgship 2>&1 | grep "Job for pkgship.service failed because the control process exited with error code." &&
    journalctl -u pkgship -n 20 | grep "\[ERROR\] The parameter 'log_path' does not exist or has no value in the configuration file '/etc/pkgship/package.ini'"
    ACT_SERVICE stop
    CHECK_RESULT $? 0 0 "Check start by systemctl failed when delete log_path."
    su pkgshipuser -c "pkgshipd start 2>&1 | grep \"\[ERROR\] The parameter 'log_path' does not exist or has no value in the configuration file '/etc/pkgship/package.ini'\""
    CHECK_RESULT $? 0 0 "Check start by pkgshipd failed when delete log_path."
