...
class TestEnv():
    ...
    def __init__(self):
        ...
        self.list_in_suite_mugen = []
        self.list_in_suite_riscv = []
    ...
    def test_list_in_suite(self,test_suite):
        if test_suite in self.suite_list_mugen:
            mugen_file = open(self.suite_cases_path+test_suite+".json",'r')
            mugen_data = json.loads(mugen_file.read())
...
class TestTarget():
    ...
    def __init__(self,list_file_name=None , testSuite=None):
        self.is_checked = 0
        self.is_tested = 0
        self.test_list = []
        self.unaval_test = []
    ...
    def CheckTargets(self,suite_list_mugen,suite_list_riscv,mugen_native = False,qemu_mode=False):
        ...
        if (not mugen_native) and (self.test_suite is None):
...
if __name__ == "__main__":
    ...
    if args.analyze is True:
        ...
        if (args.list_file is not None) and (args.test_suite is not None):
            test_target = TestTarget(list_file_name=args.list_file , testSuite=args.test_suite)
            test_target.PrintTargetNum()
            test_env.test_list_in_suite(args.test_suite)
            test_target.CheckTargets(suite_list_mugen=test_env.list_in_suite_mugen,suite_list_riscv=test_env.list_in_suite_riscv,mugen_native=args.mugen)
